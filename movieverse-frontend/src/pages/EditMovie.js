...
function EditMovie() {
  ...
  if (role !== 'admin') {
    return (
      <div className="movie-form-container">
        <h2 className="no-permission">You do not have permission to edit movies.</h2>
      </div>
    );
  }
  ...
  return (
    <div className="movie-form-container">
      ...
      <form onSubmit={handleUpdate}>
        ...
        <input type="number" value={movie.rating} onChange={(e) => setMovie({ ...movie, rating: e.target.value })} required />
        <input type="text" value={movie.posterUrl} onChange={(e) => setMovie({ ...movie, posterUrl: e.target.value })} required />
        <input type="text" value={movie.trailerUrl || ''} onChange={(e) => setMovie({ ...movie, trailerUrl: e.target.value })} placeholder="Trailer URL" required />
        <button type="submit">Update Movie</button>
      </form>
    </div>
  );
