...
function AddMovie() {
  ...
  if (role !== 'admin') {
    return (
      <div className="movie-form-container">
        <h2 className="no-permission">You do not have permission to add movies.</h2>
      </div>
    );
  }
  ...
  return (
    <div className="movie-form-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="number" placeholder="Rating" value={movie.rating} onChange={(e) => setMovie({ ...movie, rating: e.target.value })} required />
        <input type="text" placeholder="Poster URL" value={movie.posterUrl} onChange={(e) => setMovie({ ...movie, posterUrl: e.target.value })} required />
        <input type="text" placeholder="Trailer URL" value={movie.trailerUrl} onChange={(e) => setMovie({ ...movie, trailerUrl: e.target.value })} required />
        <button type="submit">Add Movie</button>
      </form>
    </div>
  );
